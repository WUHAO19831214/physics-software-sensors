...
Repository-wide gaps are Windows/Linux runtime coverage, real hardware evidence, long-duration stability, dependency security review and source-license resolution. The first documented E5 downstream integration now exists for `tracker.spot-centroid`, but it is synthetic offline replay and does not close E4 optical/device or metrology gaps. Every Sensor remains experimental and no result should be described as metrologically validated.
