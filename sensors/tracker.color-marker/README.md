...
**Demo asset pending。** 五个来源仓库的固定 commit 中没有提交可复用的 tracking 截图；本轮没有伪造或复制图片。计划从来源项目真实运行状态生成“输入帧 / HSV mask / 输出标注”三联图。见 [assets/README.md](assets/README.md) 和 [资产盘点](../../docs/asset-inventory.md)。
...
`experimental` / manifest `incubating`：已有独立 Python 算法层、统一事件 adapter、固定 golden fixture 和来源同帧比较工具；尚未完成真实摄像头 L2 验证、公开数据集、CPU/内存基准或下游接入，因此不是 validated/stable。
...
见 [benchmarks/README.md](benchmarks/README.md)。当前只完成 L0 与合成帧 L1 来源兼容测试，不声称真实实验精度。
